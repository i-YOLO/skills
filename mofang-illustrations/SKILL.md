...
1. 先确定单图 brief 或完整 shot list，并在调用工具前一次性写完全部 prompts。
...
5. 按 `references/workflow.md` 选择 `balanced`、`fast` 或 `strict` 策略；默认使用 `balanced`。
6. 默认调用内置 `image_gen`；多图任务保持一张图片一次调用，但允许把互不依赖的调用放入有界并发批次。不要用一次请求代替多个资产，不要因为“批量”切换 CLI。
7. 等当前批次全部结束后统一检查输出；将不合格资产组成返修批次，每张只改变一个变量。
8. 预览任务内联展示；正式任务按交付规则落盘并报告绝对路径、最终 prompt、执行策略和所用模式。
...
- `balanced`：默认策略。3–8 张时先生成 1 张无复杂文字、最能代表角色和风格的锚点图；通过 QA 后，以最大并发数 3 生成其余资产。
- `fast`：用户明确要求尽快、并发或快速生成时使用。首轮所有独立资产直接进入最大并发数 3 的批次。
