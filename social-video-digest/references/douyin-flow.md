...
如果提示 fresh cookies、JSON 解析失败或无可用格式，不要导入 cookies，不要要求登录，改走隔离 Chrome 公开播放路线。
...
5. 保存公开播放过程中自然加载的视频轨和音频轨。
6. 用 `scripts/probe_media.py` 校验每个轨道。
7. 如果音视频分轨，使用 `scripts/merge_av.py` 合成 `source.mp4`。
8. 合成后再次校验，必须同时有视频轨和音频轨才标记为完整视频。
